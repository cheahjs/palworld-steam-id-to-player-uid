...
const webgpuBruteForce = async (target: number) => {
  ...
  //   label: 'text hash module',
  //   code: computeShaderString,
  // });

  // const pipeline = device.createComputePipeline({
  //   label: 'text hash pipeline',
  //   layout: 'auto',
  //   compute: {
  //     module,
  //     entryPoint: 'hash',
...
<template>
  <div>
    ...
    <div v-else>
      ...
      <ul>
        <li v-for="steamId in foundSteamIds" :key="steamId"><a :href="getSteamProfileUrl(steamId)">{{ steamAccountIdToString(steamId) }}</a></li>
      </ul>
      <h3>Status</h3>
